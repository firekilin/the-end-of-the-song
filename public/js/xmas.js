  ...
  liff.getProfile ().then (function(profile) {
    $.post ('/xmas/activityList', { account: profile.userId }, (data, status) => {
      ...
      for (let i = 0;i < data.length;i ++){
        $ ('#listActivity').append (' <a class="bi bi-gift" href="' + data[i].href + '"  >' + data[i].activityName + '(' + data[i].userName + ')</a>');
      }
...
$ ('#addActivity').on ('click', () => {
  liff.getProfile ().then (function(profile) {
    $.post ('/xmas/addActivity', { account: profile.userId }, (data, status) => {
      window.alert (data);
